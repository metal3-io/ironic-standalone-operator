...
func ValidateIronic(ironic *metal3api.IronicSpec, old *metal3api.IronicSpec) error {
	...
	if err := validateIP(ironic.Networking.ImageServerIPAddress); err != nil {
		return err
	}
	...
	if len(ironic.Networking.Keepalived) > 0 {
		if ironic.Networking.IPAddressManager == metal3api.IPAddressManagerKeepalived {
		...
		for i, entry := range ironic.Networking.Keepalived {
			if entry.IPAddress == "" {
				return fmt.Errorf("networking.keepalived[%d]: ipAddress is required", i)
			}
			if err := validateIP(entry.IPAddress); err != nil {
				return fmt.Errorf("networking.keepalived[%d]: %w", i, err)
			}
			if entry.Interface == "" {
				return fmt.Errorf("networking.keepalived[%d]: interface is required", i)
			}
		}
	}
