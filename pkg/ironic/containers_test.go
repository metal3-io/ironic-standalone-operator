...
func TestNetworkingServiceContainer(t *testing.T) {
	testCases := []struct {
		...
		ExpectedNetworkDriver      string
		ExpectedNetworkingJSONHost string
	}{
		{
		...
		{
			Scenario: "networking service enabled with operator-managed endpoint",
			NetworkingService: &metal3api.NetworkingService{
				Enabled: true,
				RPCPort: 6190,
			},
			NetworkingServiceEndpoint:  "metal3-ironic-networking-service.test.svc.cluster.local",
			ExpectNetworkingEnabled:    true,
			ExpectedNetworkDriver:      "ironic-networking",
			ExpectedNetworkingJSONHost: "metal3-ironic-networking-service.test.svc.cluster.local",
		},
		{
			Scenario: "networking service with custom driver",
			NetworkingService: &metal3api.NetworkingService{
				Enabled:       true,
				RPCPort:       6190,
			...
			NetworkingServiceEndpoint:  "metal3-ironic-networking-service.test.svc.cluster.local",
			ExpectNetworkingEnabled:    true,
			ExpectedNetworkDriver:      "custom-driver",
			ExpectedNetworkingJSONHost: "metal3-ironic-networking-service.test.svc.cluster.local",
		},
		{
			Scenario: "networking service with external endpoint",
			NetworkingService: &metal3api.NetworkingService{
				Enabled:  true,
				RPCPort:  6190,
				Endpoint: "external-networking.example.com",
			},
			NetworkingServiceEndpoint:  "external-networking.example.com",
			ExpectNetworkingEnabled:    true,
			ExpectedNetworkDriver:      "ironic-networking",
			ExpectedNetworkingJSONHost: "external-networking.example.com",
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.Scenario, func(t *testing.T) {
			...
			resources := Resources{
				Ironic:                    ironic,
				APISecret:                 secret,
				NetworkingServiceEndpoint: tc.NetworkingServiceEndpoint,
			}
			podTemplate, err := newIronicPodTemplate(cctx, resources)
			require.NoError(t, err)
			...
			if tc.ExpectNetworkingEnabled {
				...
				assert.Equal(t, "true", ironicEnvVars["IRONIC_NETWORKING_ENABLED"])
				assert.Equal(t, tc.ExpectedNetworkingJSONHost, ironicEnvVars["IRONIC_NETWORKING_JSON_RPC_HOST"])
				...
				if tc.ExpectedNetworkDriver != "" {
					assert.Equal(t, tc.ExpectedNetworkDriver, ironicEnvVars["IRONIC_DEFAULT_NETWORK_INTERFACE"])
				}
			} else {
