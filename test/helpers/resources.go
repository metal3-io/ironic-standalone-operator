...
func NewSwitchCredentialsSecret(ctx context.Context, k8sClient client.Client, namespace, name string) *corev1.Secret {
	secret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: namespace,
		...
		Data: map[string][]byte{
			"username": []byte("switch-admin"),
			"password": []byte("switch-password"),
		},
		Type: corev1.SecretTypeOpaque,
	}
	err := k8sClient.Create(ctx, secret)
